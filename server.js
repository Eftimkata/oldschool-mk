...
const POSTS\_FILE = path.join(\_\_dirname, 'posts.json');
const USERS\_FILE = path.join(\_\_dirname, 'users.json');
...
res.json({ apiKey: process.env.GEMINI\_API\_KEY });
});
...
const users = await readData(USERS\_FILE);
...
users.push(newUser);
await writeData(USERS\_FILE, users);
...
try {
const posts = await readData(POSTS\_FILE);
...
const posts = await readData(POSTS\_FILE);
posts.push(newPost);
await writeData(POSTS\_FILE, posts);
